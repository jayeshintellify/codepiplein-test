...
import { getNotificationsCountsAction } from "../../redux/action/getNotificationsCounts";
...
const PdfReview2 = () => {
  ...
  const handleDownload = () => {
    const fileUrl = state?.data?.data?.files?.[1]?.fileUrl;
    if (fileUrl) {
  ...
  useEffect(() => {
    const url = state?.data?.data?.files?.[1]?.fileUrl;
    if (url) {
      setPdfUrl(url);
    } else {
        ...
        <div className="container">
          <div className="headerBg">
            ...
            <img
              ...
              alt=""
            />
            <h2>{state?.data?.data?.files?.[1]?.fileName}</h2>
          </div>
        </div>
      </div>
          ...
          <div className="mx-420 mx-auto p-0">
            <button
              ...
              onClick={() => {
                navigate("/dashboard");
                dispatch(
                  getNotificationsCountsAction.getNotificationsCountReset()
                );
              }}
            >
              {t("Continue")}
